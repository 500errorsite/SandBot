    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await edit_or_reply(
                xx,
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy SandBot dyno.**",
            )
            return repo.__del__()
        try:
        ...
        await edit_or_reply(
            xx, "`SandBot Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`SandBot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy SandBot Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**✥ SandBot Sudah Versi Terbaru**")
        return repo.__del__()
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru SandBot.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari SandBot.\
    "
    }
)
